...
class PutMotion(ReplayMotionModule):
    ...
    def start(self, hand_id, location=None, rotation=np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]])):
        ...
        if self.robot.attached_object[hand_id] is None:
            ...
            return
        if location is None:
            location = self.robot.get_global_pose()[:3] + self.robot.global_rot @ np.array([0.5, 0, 0.5])
        self.robot.action_state = self.motion_name
        self.robot.action_status = ActionStatus.ONGOING
        self.hand_id = hand_id
        self.target_location = location
        self.target_rotation = geom_utils.R_to_quat(rotation)
        self.at_stage = 0
        self.at_frame = 0
        self.obj = self.robot.attached_object[hand_id]
    
    def step(self, skip_avatar_animation=False):
        if self.at_frame == 0:
            orientation = (self.target_location - self.robot.global_trans) * np.array([1.0, 1.0, 0.0])
        ...
        if skip_avatar_animation:
            ...
            self.robot.attached_object[self.hand_id].set_pos(self.target_location)
            self.robot.attached_object[self.hand_id].set_quat(self.target_rotation)
            self.robot.attached_object[self.hand_id] = None
            self.robot.del_no_collision(self.obj)
