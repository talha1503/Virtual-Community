...
class ThrowMotion(ReplayMotionModule):
    ...
    def start(self, hand_id, orientation=None):
        ...
        if self.orientation is None:
            self.orientation = self.robot.global_rot
    ...
    def step(self, skip_avatar_animation=False):
        if self.at_frame == 0:
            self.robot.global_rot = self.orientation
        ...
        if skip_avatar_animation:
            ...
            orientation = self.robot.global_rot @ np.array([1.0, 0.0, 0.0])
            self.robot.attached_object[self.hand_id].set_qd(np.concatenate([orientation * 100, np.zeros(3)]))
        ...
        if self.at_stage == 0:
            self.at_frame += 1
            if self.at_frame == 11: # The frame when the object leaves
                self.at_stage = 1
                orientation = self.robot.global_rot @ np.array([1.0, 0.0, 0.0])
                self.robot.attached_object[self.hand_id].set_qd(np.concatenate([orientation * 100, np.zeros(3)]))
                self.robot.attached_object[self.hand_id] = None
